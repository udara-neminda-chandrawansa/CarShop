    ...
    public static void initiateDialog() {
        try (Scanner myObj = new Scanner(System.in)) {
            ...
            switch (taskNo) {
                ...
                case 3:
                    // upgrade
                    System.out.println("> Let's upgrade!");
                    // list down all car names
                    for (Car i : carList) {
                        System.out.println("> You have: " + i.carName + ": " + i.hashCode());
                    }
                    // select a car to upgrade
                    System.out.print("> To select a car to upgrade, input it's number here: ");
                    int carToUp = myObj.nextInt();

                    // validate selected car
                    for (Car i : carList) {
                        boolean validCar = false;
                        if (i.hashCode() == carToUp) {
                            validCar = true; // a valid car is selected
                            // provide upgrade options
                            System.out.print(
                                    "> Upgrade Engine: `UE` | Upgrade Transmission: `UT` | Upgrade Brakes: `UB` ");
                            myObj.nextLine();
                            String upgradeMethod = myObj.nextLine();
                            switch (upgradeMethod) {
                                case "UE":
                                    // engine upgrade needed
                                    System.out.print("> Engine Upgrade Level: ");
                                    int engineUpLvl = myObj.nextInt();
                                    i.upgradeEngine(engineUpLvl);
                                    break;
                                case "UT":
                                    // transmission upgrade needed
                                    System.out.print("> Transmission Upgrade Level: ");
                                    int transmissionUpLvl = myObj.nextInt();
                                    i.upgradeTransmission(transmissionUpLvl);
                                    break;
                                case "UB":
                                    // brakes upgrade needed
                                    System.out.print("> Brake Upgrade Level: ");
                                    int brakeUpLvl = myObj.nextInt();
                                    i.upgradeBrakes(brakeUpLvl);
                                    break;
                                default:
                                    System.out.println("> Invalid upgrade command used!");
                                    break;
                            }
                            System.out.println();
                            i.displayInfo();
                        ...
                        if (!validCar) {
                            System.out.println("You have not selected a valid car!");
                        }
                    }
                    initiateDialog();
                    break;
...
class Car {
    ...
    public Car(String carName, String carManufacturer, float carAcceleration, int carTopSpeed, float carHandling,
            int carEngineLvl, int carTransmissionLvl, int carBrakeLvl) {
    ...
    public void pointsCounter(int upgradePart) { // method to count all engine, transmission, brake points and calculate
                                                 // car performance (Acceleration, Top Speed, Handling)
    ...
    public void displayInfo() {
        System.out.println("> " + this.carManufacturer + " " + this.carName + " with level " + this.carAcceleration
                + " acceleration, " + this.carTopSpeed + "mph top speed, level " + this.carHandling + " handling.");
        System.out.println("> Engine Level: " + this.carEngineLvl + " | Transmission Level: " + this.carTransmissionLvl
                + " | Brakes Level: " + this.carBrakeLvl);
    }
}
